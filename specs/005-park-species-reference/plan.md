...
local Excel workbooks under `parkinfo/`. Scope covers server-side workbook
...
park-specific Excel workbook from the repository-local `parkinfo/` directory,
...
shadcn/ui primitives, one added Excel parsing dependency (`xlsx`), and
repository-local workbook assets in `parkinfo/`  
**Storage**: Read-only local Excel workbooks under
...
fast enough for the current workbook scale without pagination  
...
user-facing product copy MUST default to Simplified Chinese; Excel source files
must be read only on the server; preview mode MUST cap initial display at 10
records; the detail modal MUST render above all page chrome  
**Scale/Scope**: 4 preset workbook sources currently present in `parkinfo/`,
roughly 32 to 58 usable records per park after excluding header rows, 1 new
...
- [x] Frontend analysis surfaces call only the approved Route Handler and do
      not read Excel files or server-only helpers directly.
...
- [x] `research.md`, `data-model.md`, `contracts/`, and `quickstart.md`
      consistently keep Excel access behind the Route Handler boundary.
...
workbook parsing and preview/full query semantics are a separate backend
concern. Normalize workbook metadata and record-shaping helpers under
